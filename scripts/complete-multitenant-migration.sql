...
-- We'll use a comprehensive approach to ensure we catch all policies
...
-- Using a more comprehensive approach to catch any remaining policies
DO $$
DECLARE
    policy_record RECORD;
BEGIN
    -- Find and drop all policies that reference tenants.user_id
    FOR policy_record IN 
        SELECT schemaname, tablename, policyname 
        FROM pg_policies 
        WHERE definition LIKE '%tenants.user_id%'
    LOOP
