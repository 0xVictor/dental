-- Simple seed script that creates the sample data generation function
-- This avoids foreign key constraint issues by not inserting hardcoded data

-- Create a function to generate sample data for a tenant
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE NOTICE 'Error generating sample data: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;

-- Create a function to clean up sample data (useful for testing)
CREATE OR REPLACE FUNCTION cleanup_sample_data_for_tenant(tenant_uuid UUID)
RETURNS void AS $$
BEGIN
    DELETE FROM notifications WHERE tenant_id = tenant_uuid;
    DELETE FROM financial_transactions WHERE tenant_id = tenant_uuid;
    DELETE FROM staff WHERE tenant_id = tenant_uuid;
    DELETE FROM subscriptions WHERE tenant_id = tenant_uuid;
    DELETE FROM medical_records WHERE tenant_id = tenant_uuid;
    DELETE FROM appointments WHERE tenant_id = tenant_uuid;
    DELETE FROM patients WHERE tenant_id = tenant_uuid;
END;
$$ LANGUAGE plpgsql;
